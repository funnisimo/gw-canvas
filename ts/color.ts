...
function clamp(v: number, n: number, x: number): number {
...
export class Color {
  ...
  equals(other: Color | ColorBase) {
    if (typeof other === "string") {
      ...
      if (this.name && this.name === other) return true;
    } else if (typeof other === "number") {
      if (other < 0x1000) return this.toInt() === other;
    }
    const O = from(other);
  ...
  blend(other: ColorBase): Color {
    ...
    const newColor = make(
      Math.round(this.r * keepPct + O.r * pct),
      Math.round(this.g * keepPct + O.g * pct),
      Math.round(this.b * keepPct + O.b * pct),
      Math.round(O.a + this.a * keepPct)
    );
  ...
  mix(other: ColorBase, percent: number): Color {
    ...
    const newColor = make(
      ...
      Math.round(this.g * keepPct + O.g * pct),
      Math.round(this.b * keepPct + O.b * pct),
      (this.isNull() ? 100 : this.a) * keepPct + O.a * pct
    );
  ...
  lighten(percent: number): Color {
    ...
    return make(
      Math.round(this.r * keepPct + 100 * pct),
      Math.round(this.g * keepPct + 100 * pct),
      Math.round(this.b * keepPct + 100 * pct),
      this.a
    );
  }
  ...
  darken(percent: number): Color {
    ...
    return make(
      Math.round(this.r * keepPct + 0 * pct),
      Math.round(this.g * keepPct + 0 * pct),
      Math.round(this.b * keepPct + 0 * pct),
      this.a
    );
  }

  bake(_clearDancing = false): Color {
    return this;
  }
  ...
  add(other: ColorBase, percent: number = 100): Color {
    ...
    return new Color(
      Math.round(this.r + O.r * alpha),
      Math.round(this.g + O.g * alpha),
      Math.round(this.b + O.b * alpha),
      clamp(Math.round(this.a + alpha * 100), 0, 100)
    );
  }
  ...
  scale(percent: number): Color {
    ...
    return make(
      Math.round(this.r * pct),
      Math.round(this.g * pct),
      Math.round(this.b * pct),
      this.a
    );
  }
  ...
  multiply(other: ColorData | Color): Color {
    ...
    if (Array.isArray(other)) {
      if (other.length < 3) throw new Error("requires at least r,g,b values.");
    ...
    return make(
      Math.round(this.r * (data[0] / 100) * pct),
      Math.round(this.g * (data[1] / 100) * pct),
      Math.round(this.b * (data[2] / 100) * pct),
      100
    );
  }
  ...
  normalize(): Color {
    ...
    return make(
      Math.round((100 * this.r) / max),
      Math.round((100 * this.g) / max),
      Math.round((100 * this.b) / max),
      100
    );
  }
...
export function smoothScalar(rgb: number, maxRgb = 100) {
